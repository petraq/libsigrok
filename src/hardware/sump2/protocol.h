...
SR_PRIV extern const char *sump2_channel_names[];

SR_PRIV static int send_shortcommand(struct sr_serial_dev_inst *serial,
                                     uint8_t command);
SR_PRIV static int send_longcommand(struct sr_serial_dev_inst *serial,
                                    uint8_t command, uint8_t *data);
SR_PRIV static int ols_send_reset(struct sr_serial_dev_inst *serial);
SR_PRIV static void ols_channel_mask(const struct sr_dev_inst *sdi);
SR_PRIV static int ols_convert_trigger(const struct sr_dev_inst *sdi);
SR_PRIV static struct dev_context *ols_dev_new(void);
SR_PRIV static struct sr_dev_inst *get_metadata(struct sr_serial_dev_inst *serial);
SR_PRIV static int ols_set_samplerate(const struct sr_dev_inst *sdi,
                                      uint64_t samplerate);
SR_PRIV static void abort_acquisition(const struct sr_dev_inst *sdi);
SR_PRIV static int ols_receive_data(int fd, int revents, void *cb_data);
